...
void error_in_test(int num, struct Result* test_solution, struct Result* roots);
...
void test_square_equation()
{
    ...
    for (int num_of_test = 0; num_of_test < quantity_of_tests; )
    {
        ...
        if (fscanf(file_square_tests, "%lf %lf %lf %lf %lf %d", &a, &b, &c, &x1, &x2, &n_roots) != 6)
        {
